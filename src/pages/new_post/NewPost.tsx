...
function NewPost(): JSX.Element {
  ...
  const submitPost = () => {
    if (postDescription.current.value && selectedId) {
      setBtnFlag(true);
      createPost(postDescription.current.value, selectedId, token)
  ...
  const filterTopics = (event: any) => {
    if (event.target.value !== "") {
      setSuggestedTopics(
        topics.filter((item) => item.topicName.includes(event.target.value))
      );
    } else {
      setSuggestedTopics([]);
    }
      ...
      <div className="col-md-6">
        <ICXGramCard className="root-card-createpost">
          <div style={{ textAlign: "center", marginTop: "25px" }}>
            <span className="title-bold-createpost">Create Post</span>
            <div
              style={{ height: "20px", marginBottom: "20px" }}
              className="err-createpost"
            >
              {err}
            </div>
            <InputArea
            ...
            <TopicButton
              className="create-button-createpost"
              onClick={submitPost}
              disabled={btnFlag}
            >
              Create
