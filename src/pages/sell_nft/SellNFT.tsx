...
import setPrice from "../../api/set_price_nft";
import setPriceImgNFT from "../../api/set_price_img_nft";
import getMyLatestCrowdfunds from "../../api/get_latest_crowdfund";
import getMyNFT from "../../api/get_my_nft";
function SellNFT(): JSX.Element {
  ...
  const sell = () => {
    if (Number(price.current.value) > 0 && token) {
      setBtnFlag(true);
      if (feed.owner.imgUri != null) {
        setPriceImgNFT(feed.postId, price.current.value, token)
          .then((getResponse: any) => {
            if (getResponse.data.txId != null) {
              navigate("/home/ImageNFTs");
            }
          })
          .catch((error) => {
            setBtnFlag(false);
        ...
        setPrice(feed.postId, price.current.value, token)
          .then((getResponse: any) => {
            if (getResponse.data.txId != null) {
              ...
              <div className="col p-0">
                <div className="row mx-0 align-items-center">
                  <div className="col-lg-3 col-md-6 p-0">
                    <div id="profileImage">
                      {makeInitials(feed.owner.accountId.userName)}
                    </div>
                  </div>
                  <div className="col p-0">
                    <div className="row mx-0">
                      <BigText style={{ fontWeight: "600" }}>
                        {feed.owner.accountId.userName}
                      </BigText>
                    </div>
                    <div className="row mx-0">
              ...
              <div className="col p-0 text-right">
                <SubText style={{ fontWeight: "600" }}>
                  {timesAgo(
                    ...
                    Number(feed.owner.createdAt / 1000)
                  )}
                </SubText>
              </div>
            </div>
            {feed.owner.imgUri != null && (
              <div>
                <img
                  src={feed.owner.imgUri}
                  width="100%"
