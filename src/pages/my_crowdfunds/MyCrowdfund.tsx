...
function MyCrowdfunds(props: any): JSX.Element {
  console.log(props);
  ...
  const navigateToDesc = (event: any) => {
    event.target.id && navigate(`/home/wallet`, { state: event.target.id });
  };
    ...
    if (token) {
      ...
      getUnfollowedTopics(token)
        .then(async (getTopicsResponse: any) => {
          const cfs = await props.props.near.contract1.get_cfs_by_accountId({
